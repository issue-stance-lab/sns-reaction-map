...
v3の正典仕様は `templates/topic-page-v3.md` に定義されている。**必ず全文を読んでから作業を始めること**。参考実装は `docs/ai-copyright-reaction-map.html`（コミット `37e09d5` 以降の版）。

## あなたのタスク

`docs/bike-blue-ticket-reaction-map.html` を `templates/topic-page-v3.md` の8セクション構成に改修する。
...
2. 各投票選択肢が、2軸の意味的にどのあたりに位置するかを自分で判断し、`templates/topic-page-v3.md` の「4. 2Dスタンスマップ」セクションにある ai-copyright の記法（座標目安付き）にならって、bike-blue-ticket版の対応表を作成する
3. 実装後、この対応表を完了報告に明記する（座標と判断理由を含む）

## 制約・注意（重要）

- **変更してよいファイル**: `docs/bike-blue-ticket-reaction-map.html` のみ。他テーマのHTML・ポータル(index.html)・スクリプト・`templates/topic-page-v3.md` は変更しない
