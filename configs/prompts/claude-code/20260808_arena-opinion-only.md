...
**皇室典範（`koshitsu-tenpakai`）の1本だけ。** 他のテーマには触らない。

残り3本はそれぞれ専用の発注書があり、**このセッションと並行して別セッションで進めてよい**
（互いに依存しない）。作業ツリーだけは必ず分けること。

| テーマ | 発注書 | 状態 |
|---|---|---|
| 辺野古 | `20260808_henoko-opinion-only.md` | 着手可能 |
| 高齢者免許・自転車 | `20260808_elderly-opinion-only.md` | 着手可能（6論点分類は 2026-08-08 に main へマージ済み） |
| 憲法改正 | `20260808_constitutional-arena.md` | 着手可能（意見判定は PR #39 で完了。意見577件） |

全体の進め方は `20260808_denominator-opinion-only.md`（実行順の案内図）を見ること。
...
**2026-08-08 時点の main で通ることを確認済み。** 出力は次のようになる。

```text
出所: social-samples/koshitsu-tenpakai_hermes_cur_20260726.json（347件）
論点: 男系維持 vs 女系容認=104 / 旧宮家男系男子の養子制度=74 / 国会審議・成立手続き=54 /
      女性天皇と女系天皇の違い=40 / 愛子さま・皇族の地位=17 / その他・判断困難=58
投票選択肢: 6  論点カード: 5枚
OK
```

ここで差分が出たら、**先にその原因を報告して止まる**。手作業でページが書き換えられている
可能性がある。また `投票選択肢: 6` と `論点カード: 5枚` は作業後も同じでなければならない
（意見に絞っても6つの論点すべてに意見が残るため、選択肢は減らない）。
...
- `build_data_sheet.py` が最後に出す「ずれあり」の一覧から、**皇室典範が消えている**

作業前の「ずれあり」は次の6件。ここから皇室典範の行だけが消えれば成功。

```text
ずれあり  6件
  - 憲法改正論議: 意見577 に対し マップの点422
  - 辺野古高校生死亡事故: 意見が未判定（論点363件を全件で数えている）
  - 高市文春問題: 意見360 に対し マップの点359
  - 皇室典範改正: 意見283 に対し マップの点347      ← これが消える
  - 副首都法案・副首都構想: 意見229 に対し 論点の合計227 / マップの点227
  - 高齢者免許返納: 意見146 に対し マップの点211
```

**他のテーマのずれは直さない。** それぞれ別の発注書がある。
作業ツリーで実行すると「正典は最新・ページはブランチの中身」で混ざるという注意が
末尾に出るが、担当外のずれを気にする必要はない。
