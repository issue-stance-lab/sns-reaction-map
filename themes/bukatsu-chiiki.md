...
## 2026-09-22 定期収集分（新規108件・意見82件）の公開準備（課題69の型）

同日朝に課題81の収集チェックで保存した更新回（run-id `20260922_082051`、kimi-k2.6）を
`--prepare-promotion`で候補化し、オーナー承認（作業ツリー内での反映）のうえ正典へ統合した。
正典1,618件→1,726件・意見1,319件→1,401件。取得期間は2026-06-27〜2026-09-21。

- 独自性検査: 読了記録は据え置き。未読率は教員の働き方78/401=19.5%が最大で、
  4割上限内のため追い読みは不要と判断
- 語られていない争点4件: 新規のうち意見と判定された82件を既存の条件語で再検索。3件ヒット
  （体育館の利用風景2件・教員免許更新制度への言及1件）を本文で読み、いずれも争点そのものへの
  言及ではないため除外。母数のみ1,401へ更新
- 候補ページは山なみ区画が旧件数のままだったため、`build_public_registry.py`→
  `refresh_planet_section.py --for-docs`→`build_bukatsu_arena.py`の順で作り直した
  （planetとarenaが空行1行を交互に書き換える既存の揺れがあり、mainと同じarena後の形に揃えた）
- 「世論の潮目」の左ラベルが重なる不具合を発見（慎重・反対19.0%と条件付き14.0%が近接）。
  重なり防止の間隔34pxがラベル＋割合の2行ぶん（約41px）より狭かったため、
  `update_bukatsu_tide.py`のminGapを44へ変更。同じ34pxの処理は他テーマのページにもある（未対応）
- 立場の構成比の変化は最大1.6pt（中立・情報8.5%→10.1%）で、note記事の更新は不要

作業は`../isa-wt-bukatsu-0922`（ブランチ`task/bukatsu-refresh-0922`）で行った。
