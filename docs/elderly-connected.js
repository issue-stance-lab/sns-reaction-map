/* 生成AIと著作権の読書面・中心部分の配置（工程3、2026-09-23）。
   ...
   直前へ移動し、.elderly-license-connectedを付与してCSSを切り替える。
   window.ElderlyLicenseConnectedMapとdrawPanel/layout/renderの差し替えはbridge.js側の責務の
   ため、ここでは触らない。理由別X投稿・資料3タブ・年表統合・旧セクションの隠蔽（V07〜V10）は
   ページ全体の再配置を扱う工程4で追加する。 */
(function () {
  'use strict';
  var node = document.getElementById('elderly-connected-data');
  var map = window.ElderlyLicenseConnectedMap;
  if (!node || !map) return;
  var index;
  ...
  if (!data || data.theme_id !== 'elderly-license-revocation' || !bar || !planet || !panel) return;
  if (data.issues.some(function (it) { return !document.getElementById('elderly-license-revocation-reading-' + it.id); })) return;
  ...
  modes.querySelectorAll('button').forEach(function (button) {
    ...
    var label = document.createElement('span'); label.className = 'elc-mode-name';
    ...
    if (stance) {
      ...
      button.style.setProperty('--elc-stance-color', stance.color);
  ...
  if (guesses && !guesses.closest('.elc-guesses')) {
    var guessDetails = document.createElement('details');
    guessDetails.className = 'elc-guesses';
  ...
  document.body.classList.add('elderly-license-connected');
  ...
  function loadEmbeds() {
    ...
    panel.querySelectorAll('.elc-embed[open]').forEach(function (holder) {
      ...
      function resetFailed() {
        ...
        delete holder.dataset.elcWidgetRequested;
      }
      if (holder.dataset.elcWidgetRequested) {
      ...
      holder.dataset.elcWidgetRequested = 'true';
      try {
  ...
  panel.addEventListener('toggle', function (event) {
    if (event.target.matches('.elc-embed')) loadEmbeds();
  }, true);
}());
