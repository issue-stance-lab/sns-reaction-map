/* 憲法改正論議の読書面・中心部分の配置（工程3）。
   バー↔山の状態共有・読書面そのものはscripts/templates/constitutional_connected_bridge.jsが
   既に配線済み。ここではbukatsu-chiikiのbukatsu-connected.jsに合わせて、山の直前へバーと
   #modesをまとめ、#modesの中身を件数・割合つきのグリッドボタンへ作り直し、#listを読書面の
   直前へ移動し、.constitutional-connectedを付与してCSSを切り替える。
   window.ConstitutionalConnectedMapとdrawPanel/layout/renderの差し替えはbridge.js側の責務の
   ため、ここでは触らない。理由別X投稿・資料3タブ・年表統合・旧セクションの隠蔽（V07〜V10）は
   ページ全体の再配置を扱う工程4で追加する。 */
(function () {
  'use strict';
  var node = document.getElementById('constitutional-connected-data');
  var map = window.ConstitutionalConnectedMap;
  if (!node || !map) return;
  var index;
  ...
  if (!data || data.theme_id !== 'constitutional-amendment' || !bar || !planet || !panel) return;
  if (data.issues.some(function (it) { return !document.getElementById('constitutional-amendment-reading-' + it.id); })) return;
  ...
  modes.querySelectorAll('button').forEach(function (button) {
    ...
    var label = document.createElement('span'); label.className = 'ca-mode-name';
    ...
    if (stance) {
      ...
      button.style.setProperty('--ca-stance-color', stance.color);
  ...
  if (guesses && !guesses.closest('.ca-guesses')) {
    var guessDetails = document.createElement('details');
    guessDetails.className = 'ca-guesses';
  ...
  document.body.classList.add('constitutional-connected');
  ...
  function loadEmbeds() {
    ...
    panel.querySelectorAll('.ca-embed[open]').forEach(function (holder) {
      ...
      function resetFailed() {
        ...
        delete holder.dataset.caWidgetRequested;
      }
      if (holder.dataset.caWidgetRequested) {
      ...
      holder.dataset.caWidgetRequested = 'true';
      try {
  ...
  panel.addEventListener('toggle', function (event) {
    if (event.target.matches('.ca-embed')) loadEmbeds();
  }, true);
}());
