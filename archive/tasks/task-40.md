...
**状態**: 完了（2026-09-21）。あだ名30件・辺野古35件を適用・公開確認済み。原対象の残件だった
あだ名1件も対象外を確定し、当初対象はすべて解消した
**発見**: 2026-08-08

## 2026-09-21：あだ名の保留1件を対象外確定・完了

保留していた投稿（「ジャスティス切嗣というあだ名でいじめられた」、tweet_id
`2077333792331796511`）を、ブラウザで実際のXの投稿を開いて確認した。投稿者は
**「佐々木小次郎名言bot」**という、アカウント名に「bot」と明記されたジョーク投稿専用の
自動アカウントで、投稿自体も「【悲しい過去】」で始まる定型のギャグ形式だった。

2026-09-06時点で「創作・パロディの可能性が高いが、外部文脈は未確認」として保留していたが、
今回投稿者アカウント自体を直接確認したことで、必要だった外部文脈が揃った。**実在の学校での
いじめ体験ではない（対象外）と確定する。** この投稿はもともと`is_opinion: false`で
意見数には数えられておらず、対象外を確定しても表示中の件数への影響は無い。

これにより課題40の当初対象（副首都・高市・自転車青切符・高齢者免許・あだ名禁止・辺野古の
2026-07-26収集分）はすべて解消した。

## 2026-09-06：段階Dで自転車の別17件を再確認 → 課題85へ切り出し

[採用状態台帳](../quality/designs/2026-09-06-stage-d-adoption-registry.md)で自転車7/26保存72件を照合すると、同じ投稿IDが現在正典に無いものが17件あった。過去表の「未統合0」は現在のID照合と一致しない。うち3件は別IDで同じ本文があるが、除外理由の証拠にはならない。判断記録なしとして保全し、本文・来歴と過去の処理を確認してから採否を決める。今回追加していない。

直近で修復したあだ名・辺野古66件の残り1件とは別集合。以後「原対象の残り1件」と「新たに確認した自転車17件」を混ぜない。

**2026-09-21: [課題85](task-85.md)として切り出した。** 課題40は「2026-07-26の一斉収集の統合」という当初の対象がすべて解消したため完了とし、この自転車17件は別課題として引き続き追跡する。

## 2026-09-06：適用・公開後の現在地

あだ名30件・辺野古35件を正典へ適用し、既存ページへの公開反映を確認した。原対象の正典外は66→1件となり、文脈不足のあだ名1件は引き続き保留。辺野古の分類前に除かれた別ID5件は、この原対象と別集合として保全したまま今回追加していない。古い保存回の採用であり、収集予定・更新日・旧潮目比較は進めていない。
