# 課題26 Phase B-3: constitutional-amendment をトピックページv3へ展開
...
**このタスクを始める前に、課題26 Phase B-1（bike-blue-ticket）・B-2（bukatsu-chiiki）が完了・マージ済みであることをTASK_BOARD.mdで確認すること。** 未完了の場合は着手せず、状況を報告すること。
...
- 課題25の記録によれば、constitutional-amendment は過去に「ページ重複」が発生していた経緯がある。改修対象は `docs/constitutional-amendment-reaction-map.html` のみで、`docs/constitutional-amendment-dashboard.html` や `docs/constitutional-amendment-summary.html` は**別ページであり変更対象外**。誤って統合・削除しないこと
- `docs/images/` にconstitutional系の画像がPNGとWebPで重複している（例: `constitutional-amendment-manga-page-1.png` と `.webp`）。**このタスクでは画像ファイルの削除・変換は行わず、HTML内の参照はWebPのまま維持する**（重複整理は課題26 Phase Fで別途対応）
...
7. 回遊セクション（他テーマへのカード2-3枚。**constitutional-amendment以外の4テーマから選ぶ**）
...
- 新しい画像は追加しない・既存画像の変換もしない（既存の `docs/images/` 内のWebP参照のみ使用）。漫画・回遊カード画像には必ず `loading="lazy"`
- `ctx.filter` を描画アニメーション中に使わない（Safari重量化の既知の罠）
- モバイルファーストで確認（375px幅で崩れないこと）

### ブランチ運用

- mainに直接コミットしない。ブランチ `task/26-phaseb-3-constitutional` で作業し、完了後PRを作成する（マージはClaude Codeが行う）

## 完了条件・報告

1. `docs/constitutional-amendment-reaction-map.html` がv3構成になり、ローカルでブラウザ表示確認済み（コンソールエラーなし、投票→比較表示→マーカー表示が動作）
2. 投票選択肢→2Dマップ座標の対応表を、座標と判断理由付きで報告する
3. GA4 / AdSense / Supabase / OGP / Buy Me a Coffee のタグ残存をgrepで確認した結果を報告する
4. `constitutional-amendment-dashboard.html` / `constitutional-amendment-summary.html` を変更していないことを報告する
5. 変更内容のサマリーを出力する
