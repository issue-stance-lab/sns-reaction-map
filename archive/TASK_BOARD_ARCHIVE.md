...
| 課題64: 部活動テーマ「学習指導要領上の位置づけ」の未照合 | Claude Code | 2026-09-07 | 未照合だった第7節を照合し、第8節（制度の枠組み）・第9節（事故の補償）を新設。誤り3件を訂正し、未解決リスト8項目をすべて解消。全9節が本文照合済みに |
...
| 課題67: 定期計測が通常ポスト・引用RTを拾わない | Claude Code | 2026-09-22 | `x_post_views.py` の計測待ち抽出を、`投稿URL:`＋`表示回数:` 形式の通常ポスト・流入投稿・引用RTにも適用。旧形式は対象外のまま。未知の「○○実績」見出しに未計測があれば警告。テスト3本追加 |
...
- [task-79.md](tasks/task-79.md): 山なみ10テーマの構成監査 (2026-09-20完了。A-3「議論の中心」3テーマ追加・B系の数字食い違い2テーマ訂正・C系の空マーカー7テーマ分削除・C-5「調査条件」ボックス2テーマ追加をすべて本番反映。henoko-student-accidentだけ2026-09-13のオーナーの明示決定によりC-5対象外。**番号は当初「課題76」で登録したが、同日に別セッションが同じ番号で別件（会話フォローの計測漏れ）を登録・完了・アーカイブしたため、この課題を課題79へ採番し直した**)
- [task-83.md](tasks/task-83.md): GROWTH.yamlのrecurring(x-profile・gsc-review等)がダッシュボードの遅れ検知に入っていなかった (2026-09-21完了。旧LOOP.md/GROWTH_LOOP.md廃止時の課題管理棚卸しで発見。cadence欄が自由記述で機械解析できないため、項目ごとにstale_after_daysを明示させる方式でrender.pyに汎用の遅延アラートを実装しOPERATIONS.mdへ反映。副産物でx-profileのlast_run更新漏れ74日を発見・訂正。gsc-reviewは実データ確認の結果、直近90日の表示回数が全サイトで2件のみと判明し、独自ドメイン移行（課題55、Search Console物件を作り直し中）による母数不足で改善候補なしと結論。次回は課題55の登録件数回復後に再確認)
- [task-84.md](tasks/task-84.md): 公開済みページの構造(ヒートマップCanvas・漫画/投票画像)を定期的に再監査する仕組みが無い (2026-09-21完了。旧LOOP.mdの①監査からの積み残し。実装前にsmCanvasMain/smCanvasHeatが「議論の山なみ」リニューアルで死んだCSSセレクタになっている(実体はwindow.PLANET_DATAというインラインJSON)と判明し、旧チェックリストを流用せず実物に合わせて`verify_page_structure.py`を新設。チャートの実データ有無・画像/スクリプト参照切れ・JS組み立ての論点画像(課題80と同種)を検査しrun_public_checks.py経由でCIへ接続。実装中は`<script>`内JSコードの文字列結合断片(`'+xxxImgPath+'`)をsrc参照と誤検知する問題が出たため、script本文を空白化してから検査するよう修正)
- [task-53.md](tasks/task-53.md): X検索実測を1週間記録し、X API移行費用をレビューする (2026-09-21完了。期限2026-09-05を過ぎて放置されていたのをTASK_BOARD索引確認で発見。台帳(company/dashboard/x-search-usage.json、非公開)への記録自体は8/29から中断なく自動継続していたが、集計してレビューする工程だけが未実施だった。実測23日間・68回のデータで費用試算した結果、投稿のみAPI化なら月額上限3,000円の3〜4割、投稿者情報まで含めると23日平均では上限内・直近7日ペースではわずかに超える水準。切り替えるべき技術的な困りごとが記録上見当たらないため、API移行見送り・ブラウザ運用継続と結論して完了)
- [task-78.md](tasks/task-78.md): 高齢者免許返納・高市文春問題の正典がGit（公開リポジトリ）に追跡されたまま残っていた (2026-09-21完了。2026-09-20に追跡解消・外付けバックアップ・CI緑化は完了済みだったが、過去にpush済みの履歴からの完全削除だけが残っていた。影響を調査したところ、対象コミットが2,378件（全履歴の97%）、書き換えると作業ツリー25個・並行セッション36個すべてに影響することが判明。対象2ファイルは過去の使い捨てデータではなく`THEMES.yaml`の`sample_file`として現在も使用中の現役データであることも確認。投稿自体が元々公開情報・フォーク0件・今後の追跡は既に停止済みという条件と、書き換えのコストを比べ、オーナー承認のうえ履歴書き換えは見送り・現状維持で完了と判断)
- [task-37.md](tasks/task-37.md): validate_theme_seo.pyが1件落ちている (2026-09-21完了。2026-08-07発見のai-copyrightページのdateModified/THEMES.yaml updated_at不一致は、再検査したところ両方とも2026-09-20で一致し解消済みと確認。原因コミットの特定はできなかったが、日付を書き込む4つの経路（refresh_topic.pyのpromote・候補生成・bukatsu-chiiki専用・consumption-tax-cut専用）すべてを調べ、同種の不一致を生む欠陥が無いことを確認。あわせてこの検査がrun_public_checks.py経由の自動検査になったのは2026-09-02（発見時点ではまだ手動実行のみ）だったと判明し、今後の再発は自動検査で捕捉される状態になっていることを確認して完了)
- [task-40.md](tasks/task-40.md): 2026-07-26の一斉収集が正典に統合されていない (2026-09-21完了。副首都・高市・自転車青切符・高齢者免許・辺野古は既に統合済みで、残っていたあだ名禁止テーマの保留投稿1件（「ジャスティス切嗣というあだ名でいじめられた」）をブラウザで実際のX投稿を開いて確認したところ、投稿者が「佐々木小次郎名言bot」というジョーク専用の自動アカウントと判明し、実在のいじめ体験ではない（対象外）と確定して当初対象をすべて解消。同課題内で2026-09-06に見つかっていた自転車青切符の別集合（7/26保存72件中17件が正典に見当たらない問題、未解決）は[課題85](tasks/task-85.md)として切り出した)
- [task-87.md](tasks/task-87.md): takaichi投稿2件の断定的主張を一次資料と照合（誇張・不正確を検出） (2026-09-22完了。2026-09-22更新回・147件新規投稿の本文確認中に見つかった断定調の強い2件を、国会会議録検索システムAPI・週刊現代記事など一次資料と照合。投稿A「高市事務所が文書でZOOM会議・面識を認めた」は文書回答とオンライン会議参加自体は事実だが「面識」「ZOOM会議」「答弁が全て虚偽と確定」は不正確・誇張と判明。投稿B「国会で虚偽答弁を認めた」（週刊現代スクープ引用）は一次資料で確認できず、そもそも該当期間に実質審議自体が開かれていなかった。オーナー承認のうえ投稿Aをeditorial_overrideでarticle_usable=falseに変更（投稿Bは元々false）。同日別コミットで実名侮辱表現1件に適用した処理と同じ手順)
- [task-88.md](tasks/task-88.md): takaichi（高市文春問題）テーマの廃止（このサイト初のテーマ削除） (2026-09-22完了、オーナー承認。課題87の一次資料照合で「unlistedの理由（存命個人実名の疑惑・AdSense上不利）は解消していない」と裏付けられたうえ、`--prepare-promotion`が2026-08-21のunlisted化で削除済みのrefresh_at欄を前提にしておりエラーで停止することが判明。`collect_mode: event-driven`化で定期収集の催促は止めたが、オーナーから「それなら削除と同じ」「催促のループに陥る」と指摘を受けテーマ自体を廃止。公開ページ・画像25枚・専用スクリプト4本・専用テスト・`data/verification/`配下を削除し、`configs/refresh-pipeline.yaml`・`verify_builder_rebuildability.py`のBUILDERSタプル・`test_theme_hero_assets.py`のREMAINING_HEROES・`test_refresh_topic.py`の無条件import等、消しただけでは壊れる箇所を修正。`recompute_public_unreviewed.py`の`expected_canonical_files=11→10`（ユニットテストには捕捉されない静かな罠）、`decision-evidence.json`のtakaichiキー削除（Git履歴には残る）、`run_public_checks.py`のPRIVATE_DATA_TESTS除外リスト整理も対応。Git履歴からの完全消去はできない前提（課題78）で、「削除」は今後の追跡・表示から外すことを意味する。非公開正典（social-samples/）は削除せず維持。手順を[.claude/skills/retire-topic/SKILL.md](../.claude/skills/retire-topic/SKILL.md)として新設)
