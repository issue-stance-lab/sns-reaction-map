...
1. **専門用語は初出で一言そえる。** 「worktree（作業用のコピー）」のように、
   カッコで短く言い換える。用語を避けるのではなく、避けずに説明する。
2. **コマンドを渡すときは3点セットにする。** ①何をするコマンドか ②コピペで動く形
   ③**どう見えたら成功か**。3つめを省かない。成功の形が分からないと確認できない。
3. **入力待ちになるコマンドをオーナーに渡さない。自分で実行する。** エディタ（vim）や
   対話プロンプトが開くと、オーナーは抜け方が分からず止まる（`-m` 無しの `git merge` で
   繰り返し発生）。マージ・pushを含む本番反映は `release` スキルに従ってAIが実行する。
4. **結論を先に、理由を後に書く。** 経緯から書き始めない。
5. **判断を仰ぐときは、選択肢と「私ならこれ」を必ず添える。** 「どうしますか」だけでは
   判断材料がない。
6. **次にやることを1つだけ示して終わる。** 選択肢を10個並べない。
7. **できなかったこと・確認していないことは、はっきり書く。** 曖昧にぼかさない。
