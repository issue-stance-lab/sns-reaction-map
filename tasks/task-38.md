...
**状態**: 進行中（自転車青切符は 2026-08-17 に解消。takaichi は2026-09-22のテーマ廃止で対象外化により解消。ai-copyright は未対応で、実行すると巻き戻る）
**発見**: 2026-08-08
...
| ~~takaichi~~ | ~~7月26日 → 8月7日~~ | ~~7月12日 → 7月26日~~（2026-09-22テーマ廃止によりTHEMESから除去済み） |
...
**2026-08-17 自転車青切符**: ②を1テーマ分だけ実施。`THEMES` の `bike-blue-ticket` の `prev_file` / `cur_file` を `None` にして、単体実行では飛ばすようにした（`constitutional-amendment` と同じ形）。潮目は `scripts/refresh_adapters/bike.py` が更新回どうしを比較して作る。残るのは ai-copyright / 部活動など、まだ固定ファイル名を持つテーマ。
