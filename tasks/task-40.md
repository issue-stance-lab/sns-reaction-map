...
**状態**: 進行中（2026-09-06再計算：あだ名31件・辺野古35件が正典外。候補作成済み・未適用）
**発見**: 2026-08-08
...
|---|---:|---:|---:|---|
| あだ名禁止 | 45 | 14 | 31 | 28件追加候補、分類修正3件保留。現行432件は維持 |
| 辺野古 | 39 | 4 | 35 | 35件に関連・意見判定の補完案。うち既存分類変更2件。現行445件は維持 |

あだ名は本文31件、辺野古は35件を確認した。古いラベルが許可値に収まるだけでは、そのまま統合可能とはいえない。辺野古は旧39件すべてに関連・意見フラグがなかった。両テーマの公開JSONは未更新で、候補のページ・数字検査は指紋不一致で停止している。

辺野古は分類前の本文重複除外で別ID5件が落ちていたことも確認。この5件は上の35件と別集合で、今回の統合候補に追加せず保全した。復元するかは別判断。

詳細: [あだ名の結果](../quality/reviews/2026-09-06-data-repair-school-nickname-ban.md) / [辺野古の結果](../quality/reviews/2026-09-06-data-repair-henoko-student-accident.md)。次は分類案の確認と、課題59での候補生成・公開前検査。未適用のため残件数を0にしない。
