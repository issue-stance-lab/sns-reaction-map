...
**状態**: **4回目も不承認（2026-09-24通知）**。理由は1〜3回目と同じ「有用性の低いコンテンツ」。申請時点でGoogleが新ドメイン `sns-reaction-map.jp` の大半をまだ読んでいなかったことが最有力の原因（下記）。再申請の時期はオーナー判断待ち
**概要**: 審査結果追跡、通過後の広告ユニット配置設計、プロジェクトアドレスへの管理権限移行

**2026-09-24 不承認（4回目）**:
- オーナーにAdSenseから通知。`sns-reaction-map.jp` は「要確認」「サイトは広告を表示できない状態です」。
  理由は「ポリシー違反: 有用性の低いコンテンツ」で1〜3回目と同じ。所有権の確認は緑チェック。9/15申請から9日で結果
- **最有力の原因: 申請時点で、Googleが新ドメインのページの大半をまだ読んでいなかった。**
  ...
  - 通知文の「ウェブ上で一貫したプレゼンスを確立し」「ユーザーの関心を集めている必要があります」は、
    Googleがサイトをまだ読めておらず検索流入も無い状態と合う
- **ただし、これだけで説明できるとは限らない。** 1〜3回目は旧ドメイン（インデックスあり、28日間の表示27〜45回）で
  同じ理由だった。インデックスが揃っても通らない場合は、内容面の問題が残っていると考える
- **見つけた小さな問題（未修正）**: `configs/site-cases.json` の `site_pages` の lastmod が手書きのまま古い
  （`index.html` は2026-07-07のまま、実ファイルの最終変更は2026-09-24。about・disclaimer・privacy・usage・image-policy も同様）。
  3回目不承認の調査で「4. Googleへ送る更新情報が古い」とした問題が残っている。
  また、サイトマップには `/index.html` を載せているが、そのページのcanonicalは `/` で食い違っている
- **調べられなかったこと**: クラウドの作業環境からは `sns-reaction-map.jp` と `issue-stance-lab.github.io` への接続が
  ネットワーク設定で遮断されるため、公開中のサイトとSearch Consoleの9/21より後の状態は見ていない。上の数字はリポジトリ内の記録による
- **方針案（AI推奨、オーナー判断待ち）**:
  1. 管理画面の「問題を修正しました」「審査をリクエスト」は押さない
  2. Search Consoleで未登録ページのインデックス登録をリクエストする（オーナー操作）→ **2026-09-24実施済み**。
     about と未登録テーマ8件の計9件。同日の登録済みは5件（`/`・bike-blue-ticket・constitutional-amendment・disclaimer・image-policy）。
     **10/1ごろ確認**: 「ページ」の登録済み件数（目標14以上）と、9件の「前回のクロール」が日付に変わったか。
     変わっていなければ依頼だけでは足りないため、サイトマップの日付修正などへ進む
  3. テーマ10件・トップ・運営者情報がすべて「登録済み」になり、検索の表示回数が出始めてから再申請をCEOに諮る
  4. 待つあいだにサイトマップの日付を直し、旧ルートを案内ページへ置き換える（CEO決定A、課題55で未着手）。どちらも本番反映が必要
  5. 90日目標（`company/GOALS.yaml`、AdSense承認を2026-11-23まで）には、10月中旬に再申請して2〜4週間で結果が出れば間に合う
...
| 15-C 公開・再クロール確認 | 開発・データ部AI → CEO | 課題55・15-A/B後 | **CEO承認は取得済み（2026-09-15、課題54段階11、`approval-20260915-001`）。ただしSearch Consoleでの新ドメイン最新版取得確認はGSC API権限エラー（403）で未達**。オーナーのOAuth再認証等が必要 |
| 15-D AdSense再申請 | 事業部AI → CEO | **2026-09-15申請 → 2026-09-24不承認（4回目）** | 課題55で審査対象サイトを確定し、AdSenseへ新ドメインを追加・所有権確認済みであること。そのうえで15-A〜Cの証拠を品質監査が確認し、CEOが再申請を事前承認する。**すべて満たし、`sns-reaction-map.jp`をAdSenseへ登録・所有権確認・審査リクエストまで完了（`approval-20260915-002`）** |

**次にすること**: 管理画面の「問題を修正しました」「審査をリクエスト」は押さない。Search Consoleで未登録ページのインデックス登録をリクエストし（オーナー操作）、テーマ10件・トップ・運営者情報がすべて「登録済み」になってから再申請をCEOに諮る（下記「2026-09-24 不承認（4回目）」の方針案）

**2026-09-15 4回目の審査申請（詳細）**:
- 新ドメイン`sns-reaction-map.jp`をAdSenseにサイト登録。所有権確認は「AdSenseコードスニペット」方式で、ドメイン移行時から全ページの`<head>`に既に配置済みだった広告コード（`ca-pub-2542211932832864`）でそのまま確認が取れた（追加の貼り付け作業は不要だった）
- `ads.txt`（`docs/ads.txt`、`pub-2542211932832864`）も新ドメイン側に配置済みと確認済み
- Google Search Console: `sitemap.xml`は正常（2026-09-15読み込み成功、16ページ検出）。インデックス登録は申請時点で16ページ中1件のみだったが、トップページのインデックス登録はリクエスト済み。オーナー判断で「新ドメインの立ち上がり途中」と見なし、インデックスが薄いことを理由に申請を待たなかった
- リクエスト日時: 2026年9月15日23:50（JST）。ステータス「審査待ち（準備中）」
- 承認記録: `company/APPROVALS.yaml` `approval-20260915-002`
