...
**登録日**: 2026-09-20
**状態**: 進行中。課題77 Part A の本番反映（2026-09-21）で論点画像7枚の復活を本番URLで確認済み。脱落した経路そのものの再発防止は未着手
...
1. ~~課題77 Part A のマージで画像が戻ったことを本番で確認する~~ **完了（2026-09-21）**。
   本番URLで `src="images/topics/fukushuto/fukushuto-infographic-wide-*.webp"` が7件（7論点ぶん）確認できた
   （`data-img` 属性込みの単純な文字列一致では15件。builder が持つ画像と一致）
