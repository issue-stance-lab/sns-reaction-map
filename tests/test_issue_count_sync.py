...
class IssueCountSyncTest(unittest.TestCase):
    def test_nickname_map_heading_matches_public_json(self) -> None:
        ...
        if '<!-- PLANET_SECTION_START -->' in page:
            match = re.search(r'window\.PLANET_DATA\s*=\s*(.*?);</script>', page, re.S)
            self.assertIsNotNone(match)
            data = json.loads(match.group(1))
            self.assertEqual(data['totals']['opinions'], public['opinion_count'])
            self.assertEqual(sum(i['count'] for i in data['issues']), public['opinion_count'])
        else:
    ...
    def test_koshitsu_map_heading_matches_public_json(self) -> None:
        ...
        if "<!-- PLANET_SECTION_START -->" in page:
            ...
            self.assertEqual(_arena_points(ROOT, page, "koshitsu-tenpakai"), public["opinion_count"])
            return
    ...
    def test_fukushuto_map_heading_matches_public_json(self) -> None:
        ...
        if "<!-- PLANET_SECTION_START -->" in page:
            from scripts.verify_theme_page import _arena_points
            self.assertEqual(_arena_points(ROOT, page, "fukushuto"), public["opinion_count"])
            return
        match = re.search(r'<h2>SNS反応マップ</h2><span>意見([\d,]+)件 \|', page)
        self.assertIsNotNone(match)
        self.assertEqual(int(match.group(1).replace(",", "")), public["opinion_count"])
