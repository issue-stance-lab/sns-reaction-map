...
class EditorialWorkRegistryTest(unittest.TestCase):
    ...
    def test_committed_summary_and_body_free(self):
        ...
        self.assertEqual(len(ledger['records']), 220)
        self.assertEqual(ledger['counts'], dict(attempted=80, retain_candidate=69, change_candidate=16, hold=55))
        self.assertEqual(len({(r['topic'], r['record_id_hash']) for r in ledger['records']}), 220)
