...
7テーマ（ai-copyright / bike-blue-ticket / bukatsu-chiiki / consumption-tax-cut /
koshitsu-tenpakai / school-nickname-ban / fukushuto）は #issue-cards（画像＋X投稿、
*_issue_media.py 等が生成）が既にこの id を使っているため、scripts/build_planet_data.py
の「論点の一覧」（static_fallback）側では新規に付けない
（付けると同一ページでの id 重複になる）。
"""
...
class IssueAnchorTests(unittest.TestCase):
    ...
    def test_issue_cards_themes_reuse_the_existing_anchor_not_a_new_one(self) -> None:
        """issue-cardsを持つテーマ群では、論点の一覧側が新規にidを付けていないこと。
