...
Ultra-wide horizontal composition (left → center → right flow):
LEFT ZONE: 国から自治体への方針下達フロー。「国（文部科学省）」の文書アイコンから「都道府県」「市区町村」「地域クラブ」への矢印。自治体・地域クラブの矢印を破線にして「手探り・未整備」の注記。
CENTER ZONE: 移行タイムラインのバー図。「休日部活」→「平日部活」の段階移行ステップ。各ステップに「担い手？」「費用？」「制度？」の赤い疑問符フラグ。「スピード遅すぎ」と「速すぎ」の相反する吹き出しを両端に配置。
RIGHT ZONE: 4つの角丸カードを2×2グリッドで配置:
  「方針先行」「中身はこれから決まる構造」
  「自治体格差」「対応スピードがバラバラ」
  「本質批判」「チームではなくクラブを作れ」
