...
新規テーマと既存テーマのデータ更新では、投稿本文をOpenCode Go APIへ送り、
`minimax-m2.7`で2D分類する方式を標準とする。APIキーは `.env` の
`OPENCODEGO_API_KEY` から読む。

注意:
- 必ず10件テストを先に実行し、軸の逆転・系統的誤分類・エラー率を確認する。
- テーマ固有の2軸定義とFew-shotを `scripts/classify_2d_{theme}.py` に持たせる。
- 全量実行は途中結果を保存し、同じ投稿IDを再処理せず再開できるようにする。
- 外部APIへ送るのは公開SNS投稿の本文と分類に必要な最小限の情報だけにする。
- DM、限定公開投稿、秘密情報、認証情報は送らない。
...
- 投票データはSupabaseのvotesテーブルに保存（認証情報未設定のローカル開発時はlocalStorageにフォールバック）
...
### Step 4: OpenCode Go分類

テーマ固有の2D分類スクリプトを使う。以下は自転車青切符の増分分類例。

```bash
python3 scripts/classify_2d_bike_blue_ticket.py \
  --input social-samples/bike-blue-ticket_samples_new_YYYYMMDD.json \
  --output social-samples/bike-blue-ticket_2d_new_YYYYMMDD.json \
  --limit 10
...
Ollamaによる1D分類が必要なテーマでは、予備パイプラインとして以下を使う。
...
| 用途 | 推奨ツール |
|------|----------|
| SNS投稿の2D分類 | OpenCode Go API / `minimax-m2.7` |
| ローカル予備分類 | Ollama |
...
Hermesをターミナルワーカーとして使い、OpenCode Go分類スクリプトのテスト・全量実行・検証を担当させてもよい。
公開SNS投稿の本文は分類目的で外部APIへ送信できるが、DM・限定公開情報・認証情報は送らない。
