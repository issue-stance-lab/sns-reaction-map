...
**状態**: 方針承認・設計確定。**2026-09-01 に照合レイヤー「海・地下水脈」を追加し、課題15-Bを本課題へ統合した**（オーナー承認済み）。段階0〜5完了、段階6に着手できる
**次にすること**: 段階6（`build_planet_data.py` に海・地下水脈を接続）。オーナー判断不要
...
| 4 | 部活動の「沈んだ大陸」を抽出する | 設計書3.3.2の基準で最大4件。各件に一次資料URL・該当箇所・生活への影響・SNS件数と母数・確認日 | **済（2026-09-02、4件でオーナー確認済み）** | 済 |
| 5 | 部活動の「地下水脈」を抽出する | 2〜4本。各本に両側の代表投稿ID（各2件以上、現行正典に存在）・共有する懸念・それでも分かれる理由 | **済（2026-09-02、2本で完了）** | 不要 |
