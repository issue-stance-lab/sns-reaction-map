...
**次にすること**: **CEO承認を待つ（段階3-1）。** 段階0〜2はすべて完了し、公開候補
（`task/domain-cutover` ブランチ、コミット `77782e7` 時点の `docs/` の中身）を固定した。`company/APPROVALS.yaml` の
`approval-20260830-003` に、承認後具体的に何が起きるかを平易な言葉で書いた。承認が出ればAIが段階3（マージ・push・本番反映）を実行する
...
⑧`/sns-reaction-map/` 側だけを改善しても審査対象のルート（別リポジトリ
  `issue-stance-lab/issue-stance-lab.github.io`）には反映されない。**ルートは
  `scripts/build_root_index.py` が正典から生成するので、テーマ追加時は手で書き足さず再生成すること**
...
**状態**: 統合は未着手（**AdSense の結果が出るまで着手しない**）。重複の除去は 2026-08-15 に対処済み
...
**統合案（AdSense 通過後に判断）**: サイト本体をルートへ移し、トップを1枚にする。
...
**当面の運用ルール**: ルートには**入口だけ**を置く。調査方法・編集方針・数字の読み方といった
説明は `docs/about.html` に一本化し、ルートからはリンクする。ルートを触るときは
`python3 scripts/build_root_index.py --output ../issue-stance-lab.github.io/index.html --check`
を実行し、正典と一致することを確認する。
