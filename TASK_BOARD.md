...
**残作業（課題としては未起票）**: 7/26分（`fukushuto_hermes_cur_20260726.json` 308件）は旧5論点のまま未再分類。`scripts/inject_tide_widget.py` の fukushuto 定義（`issue_labels`）も旧5論点のまま。手順3の対照表とオーナー承認も未実施のため、公開ページは未変更。
