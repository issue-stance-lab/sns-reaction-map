...
最終更新: 2026-08-31（カスタムドメイン移行を課題55として追加）
...
**状態**: ドメイン取得・DNS・GitHub Pages・HTTPS設定は完了。公開HTML内のURL移行と検索エンジン移行は未完了
**優先度**: **最優先の基盤作業**。課題54の公開実装および課題15-Cより先に完了する
**正式URL**: `https://sns-reaction-map.jp/`
**公開元**: `issue-stance-lab/sns-reaction-map` の GitHub Pages。旧ルートリポジトリを同一内容の第二公開元にしない
...
- 旧URL `https://issue-stance-lab.github.io/sns-reaction-map/` は新しい正式URLへ転送される
...
**実施順**:
1. 最新の公開状態から専用worktreeを作り、URL設定・生成器・検査を修正する
2. 公開候補一式を生成し、旧URL残存ゼロと10テーマの到達性を検査する
3. Claudeの独立レビューを受け、指摘修正後の公開候補を固定してCEOが確認する
4. CEO承認後に、一本化した公開元へ一度だけ公開する。承認後に再生成しない
5. 本番の転送、メタデータ、robots、sitemap、10テーマを再検査する
6. Search Consoleへ登録し、移行確認後に課題54と15-Cを進める
...
- 旧オリジン文字列は、転送設定・移行記録など明示的な許可箇所以外の公開物で0件
- 公開直前に承認された生成物と、実際に公開する生成物が同一である
- Search Consoleで新ドメインのサイトマップが受理される

**対象外**: 3D実装、AdSense再申請、記事内容の改稿、収集データの数字修正
**次にすること**: Claudeレビュー後、専用タスクでURL正典化と自動検査を実装する。公開・pushはCEO承認まで行わない
...
| ID | 担当 | 状態 | 完了条件 |
|---|---|---|---|
| 15-A 表示・数字・2リポジトリ整合 | 開発・データ部AI | **課題55後** | ルート・本体・10テーマ・サイトマップの収集数／意見数／論点数／更新日が正典と一致し、説明文中の古い数字も検査で検出できる |
...
**次にすること**: 課題55を先に完了した後、15-Aだけに着手する。消費税・自転車・「分類済み」の定義を直し、
