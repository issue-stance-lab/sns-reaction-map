...
**状態**: 方針承認・設計確定。**2026-09-01 に照合レイヤー「海・地下水脈」を追加し、課題15-Bを本課題へ統合した**（オーナー承認済み）。段階0〜3完了。**2026-09-02のレビューで段階4は条件付きpass（要修正）、段階5は差し戻しとなり、段階6は着手不可**
**次にすること**: ①`configs/prompts/claude-code/20260902_task54-stage4-fix.md` → ②`同20260902_task54-stage5-reread.md` の順に実行する（**同時に走らせない。両方ともこの段落を書き換える**）。オーナーは2026-09-02にA案（段階5を読み直してから段階6）を決定済み
...
**その他（段階6の着手時に処理）**: 移行支持460件が水脈に1件も入っていない（指摘2）／
`vein-2` の論点IDと代表投稿の論点が不一致（指摘3）／`data/verification/` に投稿要約を入れたのは
同ディレクトリREADMEの規約違反（指摘4）／2ファイルを見る検査が0件（指摘8）。
