...
**状態**: 2026-08-21 に3回目の審査をリクエスト（審査待ち）。2回目は 2026-08-12 に不承認（理由: 有用性の低いコンテンツ）
...
### 課題49: 管理画面の「やること」をクリックで Claude セッション起動する

**状態**: アイデア段階  
**背景**: 管理画面（`admin/dashboard.html`）の「次の一手」や「対応が要ること」の項目をクリックすると、
そのタスクを実行する Claude セッションが自動で立ち上がると運用が楽になる。  
**制約**: `dashboard.html` は `file://` で開く静的ファイルのため、ブラウザだけでは CLI を起動できない。
実現には以下のいずれかが必要：
- ローカル HTTP サーバーを常駐させて POST を受け付けて `claude` を呼ぶ
- `dashboard.html` をサーバーモード（`python3 -m http.server`系）に切り替える  
...
1. 旧文書の重複と矛盾を棚卸しし、運用外のものを `archive/` へ移す
2. Website / X / note の制作物を、媒体ごとに見つけやすい構成へ段階的に移す
3. 経営ダッシュボードに目標、承認待ち、収支を統合する
4. テーマ設定とOGPに残る「あなたはどっち？」などの旧文言を、理念に合わせて見直し、ページ再生成と公開前検査を行う

**今回移動しないもの**: `docs/` / `scripts/` / `data/` / `configs/` など現行コードが参照するフォルダ。初回に大きく動かすと公開・検査・更新の経路が同時に壊れるため。
