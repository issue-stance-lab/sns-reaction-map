...
- **弱点1（体験フローの逆転）**: トピックページの情報順序が「漫画→投票→スタンスマップ→対立軸→集計表→サンプル」だが、初見ユーザーには「30秒で分かる要約」がなく、いきなり深い情報に入る
- **弱点2（新旧フォーマット混在）**: takaichi / henoko / school の3テーマが旧形式（半円チャート・2Dマップなし）のまま。サイトの第一印象がテーマによってバラつく
- **弱点3（ポータルが単なるカード一覧）**: index.html がテーマカードの羅列で、「今日の注目」「意見が最も割れているテーマ」などの編集的な入口がない
- **弱点4（重さ）**: images 45MB、各ページ100-186KB のインラインHTML。PNG/WebP重複も残存（constitutional系）。課題14未着手のまま画像が増え続けている
- **弱点5（行き止まりページ）**: theme-preview.html、constitutional-amendment-dashboard.html、*-summary.html、takaichi-reaction-heatmap.html 等の旧世代ページが残存し、SEO・回遊上のノイズ
**構成案（トピックページの標準フォーマット v3）**:
1. ヒーロー（画像+テーマの問い1行+「30秒サマリー」3点箇条書き）
2. 漫画で読む対立（現行維持）
3. 投票（現行維持。投票後に「SNS分布との比較」を即表示）
4. 2Dスタンスマップ（現行維持。投票結果を自分の位置としてマップ上にプロット表示）
5. 象限別の代表的な声（現行の「代表サンプル」を象限ナビ付きに再構成）
6. 論点整理（対立軸の表を「争点カード」形式に）
7. 次のテーマへの回遊（「このテーマに投票した人はこれも見ています」）
...
- Phase F: ✅ 完了（2026-07-05）。PNG重複14枚削除（約25MB削減）、v3形式4テーマのマンガモーダル `<img>` に `loading="lazy"` 追加。ブランチ `task/26-phasef-performance` をマージ済み（d8e8afd）。セッションプロンプト: `configs/prompts/hermes/20260705_task26-phasef-performance.md`
...
| 課題26: サイト構成改革（v3フォーマット） | ワーカーAI / Claude Code | 2026-07-04 | **Phase A〜F 全完了** | Phase F: PNG14枚削除+lazy loading追加（d8e8afd）。課題26全フェーズ完了 |
