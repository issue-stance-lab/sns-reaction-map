...
最終更新: 2026-08-10（課題44の再設計をビルダー・検査と噛み合わせた）
...
**状態**: 確認待ち（2026-08-09、トップページと生成AIと著作権ページで試作済み）

**試作中の方向**:
- Anthropic公式 `frontend-design` の指針を使い、汎用的な円グラフや番号付きカードではなく、サイト固有の「問いの背骨」と左右へ分かれる反応経路を設計する
- 縦方向=論点、中央線から左=権利保護・規制、右=AI活用・推進、中央付近=中立、中央線からの距離=意見の強さとして表す
- 点の選択による元Xポストへのリンク、論点フィルター、投票位置マーカーは維持する
- 最初に生成AIと著作権ページで確認し、承認後に共通部品化と全テーマ展開を検討する
- トップページは11テーマを一覧する「論点アトラス」、テーマページは一覧の下に既存の投稿詳細図を置く二層構成で試作する
