...
> **2026-08-28 追記｜図の論理幅は800pxにすること。**
> この文書の図は幅1200pxで作っているが、noteが本文の図を表示する幅はPCで620px、
> スマホで375pxしかない。1200pxで作ると、図の中の13pxの文字はスマホで4pxになり読めない。
> 第2回で作り直した際の規定は `bukatsu-chiiki2-image-prompts.md`「文字の大きさ」を参照。
> 論理幅800px、図の中の最小フォント24px、書き出しは deviceScaleFactor 3。
> 第1回の図をこのまま作り直す予定はないが、**手を入れるときは必ずこの規定に合わせること。**
