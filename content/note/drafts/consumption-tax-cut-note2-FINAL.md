...
消費税減税をめぐる「大綱」には、対象範囲も財源も、もう書いてあります。そう見えます。

実際に読むと、2つの重みはまったく違いました。対象範囲は、実施日と税率まで数字で確定していました。財源は「特例公債には頼らない」という一文があるだけで、金額はこれからでした。
...
対象範囲では、賛成が多数派でした。財源は、賛成と反対がほぼ並びました。同じ大綱の中でも、論点によって決着のつき方が違うことが、この2つのグラフの差になっています。
...
反対する投稿の多くが挙げていたのは、原則が無いことへの不満ではありませんでした。その先です。歳出削減でまかなうなら社会保障が削られるのではないか。表向きは減税でも、埋め合わせで別の増税が来るのではないか。
...
財源は違います。「原則」から「金額」に変わる瞬間が、まだ先にあります。令和9年度の予算編成過程で、何を削り、何を見直すかが具体化されたとき、財源に慎重な人たちが待っていた答えが、初めて出ます。
