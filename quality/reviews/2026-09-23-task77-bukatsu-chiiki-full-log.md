...
[移植手順を改訂](../designs/2026-09-22-task77-connected-layout-rollout-guide.md)し、
...
**あえて変えなかった2点（要オーナー確認、のちに両方とも対応・解消）**: (1) `bukatsu-check`
（制度4項目）は論点idのタグ付けが無く統合先が無いため非表示にせず通常表示のまま維持
→ 2026-09-23中にオーナー承認のうえタグ付け・統合済み。(2) 山の色は消費税の単色化
（選んだ立場の色一色）に寄せず、部活独自の「論点ごとの立場色＋海面下の演出」を維持
→ オーナー指示「消費税に揃えて」を受け、同日中に選択立場の色1色方式へ変更済み。
