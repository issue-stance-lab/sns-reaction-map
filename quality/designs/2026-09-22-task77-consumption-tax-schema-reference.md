...
[別テーマへの適用手順](2026-09-22-task77-connected-layout-rollout-guide.md)を補う**データ構造だけの資料**。画面仕様の代わりにはならない。
2026-09-23追記: 移植時は同手順の工程別完了条件・V01〜V12・旧セクション対応表を必読とし、消費税の公開画面とCSS・描画JS・ページ全体の配置JSも読む。本書の接続関係だけを再現して、見た目の移植完了とはしない。
