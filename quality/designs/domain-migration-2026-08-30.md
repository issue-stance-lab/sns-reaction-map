...
**2026-08-31 に破棄。** 実際に設定された公開元は本文の想定と異なる（`sns-reaction-map` リポジトリの
GitHub Pagesにカスタムドメインを設定する形で、公開専用リポジトリへの切り替えは行わなかった）。
旧URLがパスを保った301で転送されることを実測で確認し、こちらの構成の方が優れていたため、
**「切り替え順序」「旧URLの扱い」「安定後の公開データ基盤」（公開専用リポジトリへの同期構成）の各節は破棄する**。
`scripts/sync_public_site.py` は使わない。現行の実施順は `TASK_BOARD.md` の課題55を正典とする（本ファイルの後継）。
「Search Console」「AdSense」の各節は方針として有効。**「公開前ゲート」節は多くは方針として有効だが、
手順1・2（`scripts/sync_public_site.py` / `tests/test_sync_public_site.py` を実行するもの）は
削除済みで実行できない。手順として使わないこと**（現在の手順はTASK_BOARD.mdの課題55）。
以下は経緯として残す。
