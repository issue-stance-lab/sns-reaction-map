...
> **2026-08-31 追記**: 実際に設定された公開元は本文の想定と異なる（`sns-reaction-map`
> リポジトリのGitHub Pagesにカスタムドメインを設定する形で、公開専用リポジトリへの
> 切り替えは行わなかった）。旧URLがパスを保った301で転送されることを実測で確認し、
> こちらの構成の方が優れていたため、**「切り替え順序」「旧URLの扱い」
> 「安定後の公開データ基盤」の各節は破棄する**。現行の実施順は `TASK_BOARD.md` 課題55
> を正典とする。「公開前ゲート」「Search Console」「AdSense」の各節は方針として有効。
