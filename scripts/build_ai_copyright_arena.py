...
# configs/planet/ai-copyright.yaml の stances[].key と対応させる。
# 集計(count/color)は正典から取得済みの値をそのまま使い、ここでは新たに数えない。
STANCE_GLANCE_META = {
    "規制・制限強化支持": {
        "short": "規制支持", "icon": "!", "bg": "#FFEEE9", "shadow": "rgba(255,84,38,.22)",
        "desc": "生成AIの利用や学習には、著作権を守るための規制が必要だとする投稿",
    },
    "中立・情報": {
        "short": "中立・情報", "icon": "?", "bg": "#F1F3F5", "shadow": "rgba(100,116,139,.22)",
        "desc": "賛否を示さず、ニュースや技術の紹介にとどまる投稿",
    },
    "推進・活用支持": {
        "short": "推進支持", "icon": "✓", "bg": "#E8F0FE", "shadow": "rgba(7,94,242,.22)",
        "desc": "規制よりも、AIの活用や技術の発展を優先すべきだとする投稿",
    },
}
...
def stance_glance(stances: list[dict], opinions: int) -> str:
    ...
    stances は bpd.build("ai-copyright")["stances"]（configs/planet/
    ai-copyright.yaml の3区分に正典の件数を足したもの）をそのまま使う。
    ここで新たに集計しない。投票データへの書き込みは行わない。
    """
    restriction_count = next((int(s["count"]) for s in stances if s["key"] == "規制・制限強化支持"), 0)
    promotion_count = next((int(s["count"]) for s in stances if s["key"] == "推進・活用支持"), 0)
    ...
    ratio = restriction_count / promotion_count if promotion_count else 0.0
    script = f"""<script>
(function(){{
...
<div class="panel-title"><h2 id="stance-glance-title">生成AIと著作権、規制派は推進派の{ratio:.1f}倍</h2><span>本文の前に、まず内訳を</span></div>
<p class="sg-lead">規制・制限強化を支持する投稿は{restriction_count:,}件で、全体の過半数を占めます。推進・活用を支持する投稿{promotion_count:,}件と比べると、{ratio:.1f}倍の差があります。</p>
<div class="sg-headline"><b>{opinions:,}</b>件の意見の、立場ごとの内訳です</div>
{bar}
<div class="sg-pick"><p class="sg-pick-label">詳しく知りたい立場を押すと</p>
<p class="sg-pick-hint">その立場の投稿にある考え方を表示します</p>
...
<p class="sg-note">※ ボタンは内容の確認用です。投票としては数えません。投票はこのページ下部から参加できます。</p>
</div>
</aside>
{script}
{STANCE_GLANCE_END}"""
...
def apply_ai_copyright_stance_glance(page: str) -> str:
    """ヒーロー直後に、立場の内訳＋「まず、あなたは？」を貼り直す。

    このページの定例更新経路(build()のPLANET_SECTION分岐、apply_background()のみ
    呼ぶ設計)はRESEARCH_CONDITIONS_START直前の領域を素通りする(447-454行目の
    コメントに明記された既知の制限)ため、放っておくと件数が更新されず古いまま
    残る。他テーマの後付け補完処理と同じ「除去してから再挿入」の型で毎回貼り直す。
    """
    if __package__:
        from .build_planet_page_preview import bpd
    else:  # python3 scripts/build_ai_copyright_arena.py
    ...
    if page.count(STANCE_GLANCE_START) != 1 or page.count(STANCE_GLANCE_END) != 1:
        raise BuildError("内訳セクションのマーカーが1組でない")
    if '<h2 id="stance-glance-title">' not in page:
        raise BuildError("内訳セクションの見出しがページにない")
    ...
    if seg_count != len(stances):
        raise BuildError(f"内訳バーの区画が{len(stances)}個でない: {seg_count}個")
    if btn_count != len(stances):
        raise BuildError(f"「あなたは？」ボタンが{len(stances)}個でない: {btn_count}個")
    if page.index(STANCE_GLANCE_START) > page.index(STANCE_GLANCE_ANCHOR):
        raise BuildError("内訳セクションが調査条件より後ろにある(ヒーロー直後に置くこと)")
...
def build(
    *,
    ...
    if "<!-- PLANET_SECTION_START -->" in before:
        ...
        page = apply_ai_copyright_stance_glance(page)
