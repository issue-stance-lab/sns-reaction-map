...
既知の積み残し: ページの「世論の潮目」ウィジェット（`datasets.issue`）だけは
2026-07-26 の旧5論点で書かれたままで、この定義に対応するラベルがない。
次にウィジェットを更新するときは 7/14・7/26 の Hermes 結果をこの論点で分類し直す必要がある
（stance 側は旧分類器と同じ3択なのでそのまま連続する）。
"""
