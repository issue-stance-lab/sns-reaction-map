...
REACTION_TYPES = ["追悼", "批判", "懸念", "支持", "反発", "制度論", "情報共有", "未確認", "対象外"]
...
- 「追悼」「黙祷」「慰霊」だけが中心で批判対象がない投稿は、criticized_target を「なし」、reaction_type を「追悼」にする。
- 文科省判断そのものへの支持/反発、学校・引率への批判、反基地運動/政治利用批判、報道批判、安全管理論を混同しない。
- URL共有、ニュース見出しだけ、単語だけ、投稿者本人の評価がないものは「事実確認・情報共有」または「その他・分類保留」にし、article_usable は false。
- 未確認証言、断定的な加害者扱い、強い個人攻撃、差別的表現は risk を medium 以上、必要なら high にする。
...
issue候補: {issues}
reaction_type候補: {reaction_types}
...
def normalize(item: dict) -> dict:
    ...
    return {
        ...
        "reaction_type": pick("reaction_type", REACTION_TYPES, "対象外"),
...
def harmonize_classification(result: dict) -> dict:
    ...
    if category == "文科省判断支持・政治的中立性違反を指摘":
        ...
        fixed["issue"] = "安全管理"
        fixed["reaction_type"] = "批判"
    elif category == "追悼・慰霊への共感":
        fixed["target"] = "追悼・慰霊"
        fixed["actor_target"] = "追悼・慰霊"
        fixed["criticized_target"] = "なし"
        fixed["stance_to_target"] = "支持"
        fixed["issue"] = "追悼"
        fixed["reaction_type"] = "追悼"
    elif category == "政治利用・反基地運動批判":
        fixed["target"] = "反基地運動"
        fixed["actor_target"] = "反基地運動"
        fixed["criticized_target"] = "反基地運動"
        fixed["stance_to_target"] = "批判"
        fixed["issue"] = "政治利用"
        fixed["reaction_type"] = "批判"
    elif category == "事実確認・情報共有":
        fixed["reaction_type"] = "情報共有"
        fixed["article_usable"] = False
    elif category in {"未確認・過激表現", "その他・分類保留"}:
        fixed["article_usable"] = False
    return fixed
