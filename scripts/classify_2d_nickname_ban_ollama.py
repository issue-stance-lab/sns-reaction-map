#!/usr/bin/env python3
"""
あだ名禁止問題の2次元スコア分類（Ollama qwen2.5:7b版）

使い方:
  python3 scripts/classify_2d_nickname_ban_ollama.py --test   # 20件テスト
  python3 scripts/classify_2d_nickname_ban_ollama.py          # 全量344件
...
OUTPUT_FILE = PROJECT_ROOT / "social-samples" / "school-nickname-ban_2d_classified_ollama.json"
...
stance_culture（何を優先するか）: -2.0〜+2.0 の0.5刻み
  +2.0 = 学校の管理・配慮を最優先。いじめ防止のためルール整備は当然
  +1.0 = やや管理・配慮寄り。一定のルールは必要
   0.0 = 不明・どちらでもない・言及なし
  -1.0 = やや自由・文化寄り。過剰な管理に違和感がある
  -2.0 = 子どもの自由・呼称文化を最優先。管理主義的・息苦しい
...
{{"is_opinion": true, "stance_ban": 0.0, "stance_culture": 0.0, "emotional_intensity": 0.0, "confidence": 0.8, "summary": "20字以内"}}"""
...
def main():
    parser = argparse.ArgumentParser(description="school-nickname-ban 2D分類 (Ollama qwen2.5:7b)")
    ...
    print(f"Classifying {len(samples)} posts via Ollama ({MODEL})...", flush=True)
    if args.test:
