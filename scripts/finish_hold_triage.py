...
def merge_triage(summary,input_path,output_path):
    ...
    for row in summary['records']:
        ...
        else:group=row.get('reason_group',row['primary_reason_hint']);method=row.get('grouping_method','keyword_hint_only')
    ...
    refs=summary.get('triage_sources',[])+[{'input_sha256':sha(input_path),'output_sha256':sha(output_path),'records':len(labels)}]
    return {**summary,'records':records,'reason_group_counts':dict(counts),'grouping_methods':dict(methods),'reason_groups_by_topic':dict(topics),
            'triage_sources':refs,
            'triage_input_sha256':sha(input_path),'triage_output_sha256':sha(output_path),'triage_scope':f"{methods['individual_saved_reason_triage']} saved reasons reviewed individually; other entries remain explicitly labeled keyword hints. No body reread or adoption change."}
