...
// を検証した結果、docs/constitutional-connected-page.js側が独自クイズ描画へ差し替える際に
// #quizを一度detachして中身を作り直すため、bridge.js側の委譲は実質到達しない設計だったと
// 判明した（同ページ.js側のshowQuestion()が自前でmap.selectIssue()を呼ぶため機能的には
// 重複でもある）。constitutional-amendmentではpage.js側のshowQuestion()に一本化し、ここでは実装しない。
(function(){
  ...
  function readingTemplateFor(id){
    return document.getElementById('constitutional-amendment-reading-' + id);
  }
  function fillMetrics(root, issue){
    var m = modeById[st.mode], n = m.counts[issue.id] || 0;
    var count = root.querySelector('[data-ca-count]');
    var mode = root.querySelector('[data-ca-mode]');
    var ratio = root.querySelector('[data-ca-ratio]');
    var zero = root.querySelector('[data-ca-zero]');
  ...
  function renderReading(id){
    ...
    panel.setAttribute('data-ca-issue-id', id);
    panel.innerHTML = '';
  ...
  drawPanel = function(){
    if (st.landed === null) { document.getElementById('panel').removeAttribute('data-ca-issue-id'); legacyDrawPanel(); return; }
  ...
  // 書き換えない）。
  var caVisual = null, caAnimation = 0;
  var legacyLayout = layout;
  layout = function(modeId){ return caVisual && modeId === st.mode ? caVisual : legacyLayout(modeId); };
  ...
  // configs/planet/constitutional-amendment.yamlの立場配色をそのまま使う。
  ...
  function caAnimate(from, to){
    cancelAnimationFrame(caAnimation);
    if (reduce){ caVisual = null; render(); return; }
    ...
    function frame(now){
      ...
      caVisual = Object.assign({}, to, {live: keys.map(function(id){
      ...
      render();
      if (t < 1) caAnimation = requestAnimationFrame(frame);
      else { caVisual = null; caAnimation = 0; render(); }
    }
    caAnimation = requestAnimationFrame(frame);
  }

  // 連打時、前のアニメーションの最新の形（caVisual）から次の目的地へつなぐため、
  // 常に最後に押した立場へ収束する（前のアニメーションへ戻らない）。
  morphTo = function(toMode){
    if (!modeById[toMode] || toMode === st.mode) return;
    var from = caVisual || legacyLayout(st.mode);
    ...
    caAnimate(from, legacyLayout(toMode));
  ...
  // land()が書くhashは論点の生id（例: #constitutional-amendment-general）だが、旧代表投稿・授業節の
  // 既存リンクは別の名前空間（例: #issue-constitutional-amendment-learning-data）を使っている。
  // 工程4で旧代表投稿を通常画面から隠すため、実際に残る消費先は授業節の論点リンクのみだが、
  ...
  function readingRenderedFor(i){
    return i !== null && !!document.getElementById('panel').querySelector('.ca-selected-head');
  }
  ...
  window.ConstitutionalConnectedMap = Object.freeze({
    getState: function(){
      var stance = D.stances.find(function(s){ return s.key === st.mode; });
